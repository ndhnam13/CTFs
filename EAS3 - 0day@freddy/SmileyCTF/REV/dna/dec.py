import marshal, types, dis, textwrap
import sys
...
results = []  # để lưu kết quả dạng (index, key, code_obj)

for idx, cur in enumerate(unlucky):
    found = False
    for key in range(256):
        # XOR mỗi byte
        data = bytes([b ^ key for b in cur])
        try:
            code_obj = marshal.loads(data)
            if isinstance(code_obj, type((lambda:0).__code__)):
                # Gán tạm vào hàm để có thể disassemble/chạy thử
                def f(): pass
                f.__code__ = code_obj
                print(f"#{idx}: key = {key} (char={repr(chr(key))})")
                print("Disassembly of code object:")
                dis.dis(f)
                attrs = {
                    'co_name': code_obj.co_name,
                    'co_argcount': getattr(code_obj, 'co_argcount', 'N/A'),
                    'co_consts': code_obj.co_consts,
                    'co_names': code_obj.co_names,
                    'co_varnames': code_obj.co_varnames,
                }
                print("Attributes of code object:", attrs)
                print("-" * 50)
                results.append((idx, key, code_obj))
                found = True
                break
        except Exception:
            continue
    if not found:
        print(f"Phan tu #{idx}: cant find valid keys.")
...
for cur in unlucky:
    for key in range(0,256):
        try:
            def f():
                return
            f.__code__ = marshal.loads(bytes([b ^ key for b in cur]))
            print(chr(key),end="")
            #f()
